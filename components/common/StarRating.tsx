/**
 * @fileoverview StarRating component for displaying product or seller ratings.
 * Renders filled, half-filled, and empty stars with optional numeric value and review count.
 * Uses SVG gradient for half-star rendering with unique IDs per product.
 ...
 */
interface StarRatingProps {
  /** The rating value to display (supports decimals) */
  rating: number;
  /** Maximum number of stars (default: 5) */
  maxRating?: number;
  /** Size variant affecting star dimensions */
  size?: 'sm' | 'md' | 'lg';
  /** Whether to show the numeric rating value */
  showValue?: boolean;
  /** Optional review count to display in parentheses */
  reviewCount?: number;
  /** Unique product ID for SVG gradient IDs (prevents conflicts) */
  productId?: string;
}
    ...
    <div className="flex items-center gap-2">
      ...
      {showValue && (
        <>
          ...
          {reviewCount && (
            <span className={`${textSizeClasses[size]} text-gray-500`}>({reviewCount})</span>
          )}
        </>
      )}
    </div>
