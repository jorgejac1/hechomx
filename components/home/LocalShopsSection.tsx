/**
 * @fileoverview Local Shops Section Component
 * Displays artisan shops based on the user's selected state/location. Features location-based
 * filtering with localStorage persistence, shop cards with cover images and details, and
 * a location toggle to explore shops from different Mexican states.
 * @module components/home/LocalShopsSection
 */
...
/**
 * @interface LocalShop
 * Represents an artisan shop with location and branding information
 */
interface LocalShop {
  ...
  name: string;
  /** URL-friendly username for the shop profile */
  username: string;
  /** URL of the shop logo image */
  logo: string;
  /** URL of the shop cover/banner image */
  coverImage: string;
  /** Mexican state where the shop is located */
  state: string;
  /** City within the state */
  city: string;
  /** Shop's craft specialty description */
  specialty: string;
...
/**
 * Retrieves the user's selected state from localStorage.
 * @returns {string} The user's state or 'Ciudad de Mexico' as default
 */
...
/**
 * Renders a section displaying local artisan shops based on user location.
 * Features location toggle and shop cards with masonry-style layout.
 * @returns {JSX.Element} The LocalShopsSection component
 */
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-sm sm:text-base font-bold mb-0.5 flex items-center gap-1.5">
                          ...
                          {shop.verified && (
                            <svg
                              className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-400 shrink-0"
                              fill="currentColor"
                              viewBox="0 0 20 20"
                            >
                              <path
